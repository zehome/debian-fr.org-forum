...
if ( $post_id && !$view && !$phpbb_seo->set_do_redir_post()) {
	$phpbb_seo->seo_opt['zero_dupe']['redir_def'] = array(
		'ys' => array('val' => $solved_id, 'keep' => true),
		'ns' => array('val' => $unsolved, 'keep' => true),
...
// BEGIN Topic solved
  ...
  if($ok_solve || $ok_unsolve)
  {
    ...
    {
      $u_set_solved = append_sid("{$phpbb_root_path}viewtopic.{$phpEx}", 'f=' . $forum_id . '&amp;t=' . $topic_data['topic_id'] . '&amp;p=' . $row['post_id'] . '&amp;ns=1', true);
    }
    else if($ok_solve && !$topic_data['topic_solved'])
    {
      $u_set_solved = append_sid("{$phpbb_root_path}viewtopic.{$phpEx}", '&amp;f=' . $forum_id . '&amp;t=' . $topic_data['topic_id'] . '&amp;p=' . $row['post_id'] . '&amp;ys=' . $row['post_id'], true);
    }
  }
// END Topic solved
